...
fetchTweet()
  └─ fetchFromFxTwitter() → tweet（正文 + article）
      └─ [新增] maybeMergeAuthorComments(tweet)
            ...
            ├─ ③b 补齐完整文本：GraphQL full_text 会截断某些回复(实测截在"❼")
            │    → 每条自回帖用 FxTwitter 单推接口取完整文本,取更长者
            ├─ ④ LLM 过滤：只保留「文章正文延续」评论,丢闲聊/广告
            │    prompts/comments/filter-replies.md
            ├─ ⑤ 合并：直接拼进正文(\n\n 分隔,无标题无引用标记)
            │    + 评论媒体 [IMG:N]/[VIDEO:N] 重索引
            └─ ⑥ 任一失败 → 原样返回，绝不阻塞归档
```
...
- **无标题、无 `---`、无 blockquote**——正文与评论直接 `\n\n` 拼接，读起来像一篇连续文章（正文 `👇` 后直接接 `❶❷❸…`）。
- v1 曾用 `---` + `**作者在评论区的补充**` + blockquote，用户反馈不要这种展示，改为直接拼接。
